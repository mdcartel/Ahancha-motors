		...
		const fetchStats = async () => {
			try {
				...
				setStats({
					...
					featuredVehicles: vehicles.filter((v: any) => v.featured)
						.length,
					newVehicles: vehicles.filter(
						(v: any) => v.condition === "New"
					).length,
					usedVehicles: vehicles.filter(
						(v: any) => v.condition === "Used"
					).length,
				...
				<div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8">
					<div>
						...
						<p className="text-gray-500 mt-1">
							Welcome to Ahancha Motors Dealership Admin
						</p>
					</div>
						...
						<div className="p-6">
							<div className="space-y-6">
								{recentActivity.map((activity, index) => (
									...
									>
										<div
											className={`p-2 rounded-lg ${activity.bgColor} text-white mr-4 mt-1`}
										>
											{activity.icon}
										</div>
