...
export async function GET(request: NextRequest) {
  try {
    ...
    similarVehicles.sort((a, b) => {
      const scoreA = [
        make && a.make === make ? 3 : 0,
        model && a.model === model ? 2 : 0,
        bodyType && a.bodyType === bodyType ? 1 : 0
      ].reduce((sum, val) => sum + val, 0);
      
      const scoreB = [
        make && b.make === make ? 3 : 0,
        model && b.model === model ? 2 : 0,
        bodyType && b.bodyType === bodyType ? 1 : 0
      ].reduce((sum, val) => sum + val, 0);
